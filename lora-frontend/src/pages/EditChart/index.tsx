...
const EditChartPage: React.FC = () => {
      ...
      if (res.data) {
        ...
        if (res.data.genChart) {
          try {
            ...
            // 设置代码编辑器的内容
            setChartCode(JSON.stringify(parsedOptions, null, 2));
          } catch (e) {
  ...
  // 当chartCode变化时，自动更新预览（可选的实时预览功能）
  useEffect(() => {
    if (chartCode && chartCode.trim() !== '{}') { // 只有在代码不是初始空状态时才更新
      try {
        const parsedOptions = JSON.parse(chartCode);
        setPreviewOptions(parsedOptions);
      } catch (error) {
        // 静默处理错误，不显示错误信息，避免频繁的错误提示
        console.debug('图表代码格式错误，跳过实时预览:', error);
      }
    }
  }, [chartCode]);
  ...
  const updateChartPreview = () => {
    try {
      ...
      console.error('图表代码解析失败:', error);
      message.error('图表代码格式错误，请检查JSON格式');
    }
  };
  ...
  const handleRetry = async () => {
    ...
    if (!isAdmin) {
      if (userScore === undefined || userScore < 10) {
        message.error("积分不足，每次重试需要消耗10积分，请先充值");
        return;
      }
    }
