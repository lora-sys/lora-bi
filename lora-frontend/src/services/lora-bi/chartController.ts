...
export async function genChartByAiAsyncUsingPost(
  ...
  return request<API.BaseResponseBiGenVO_>("/api/chart/gen/async", {
    method: "POST",
    params: {
      ...params,
    },
...
export async function genChartByAiAsyncMqUsingPost(
  ...
  return request<API.BaseResponseBiGenVO_>("/api/chart/gen/async/mq", {
    method: "POST",
    params: {
      ...params,
    },
...
/** retryChart POST /api/chart/retry/${param0} */
export async function retryChartUsingPost(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.retryChartUsingPOSTParams,
  options?: { [key: string]: any }
) {
  const { id: param0, ...queryParams } = params;
  return request<API.BaseResponseBoolean_>(`/api/chart/retry/${param0}`, {
    method: "POST",
    params: { ...queryParams },
    ...(options || {}),
  });
